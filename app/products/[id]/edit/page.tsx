...
export default function EditProductPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    //     return;
    //   }
    //   // for numbers: ensure difference and not NaN
    //   if (typeof existing === "number") {
    //     if (!Number.isNaN(parsed) && parsed !== existing) payload[key] = parsed;
    //     return;
    //   }

    //   if (parsed !== existing) payload[key] = parsed;
    // };

    // addIfChanged("Name", form.Name);
    // addIfChanged("Description", form.Description);
    // addIfChanged("Price", form.Price, (v) => (v === "" ? "" : parseFloat(v)));
    // addIfChanged("Stock", form.Stock, (v) => (v === "" ? "" : parseInt(v, 10)));
    // addIfChanged("CategoryId", form.CategoryId, (v) =>
    //   v === "" ? "" : parseInt(v, 10)
    // );
    // addIfChanged("BrandId", form.BrandId, (v) =>
    //   v === "" ? "" : parseInt(v, 10)
    // );
    ...
    updateProduct.mutate(payload, {
      ...
      onError: (e) => {
        toast.error("Product edit failed:" + e.message);
      },
    });
  };
